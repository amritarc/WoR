...
def main(args):
    ...
    FORGE_INDS = list(range(args.N_train, args.N_train + args.N_forge))
    M = args.M
    ...
    tb_folder = "./tensorboard/{}-{}-halfhalf-type{}-Ntrain{}-Nforge{}-B{}-LR{}-n{}-M{}".format(
        DATASET, MODEL, FORGE_TYPE, args.N_train, args.N_forge, args.batch_size, args.lr, args.n, M
    )
    ...
    valid_dataset = datasets.MNIST(root=data_folder, 
                                ...
                                transform=transform_fn,
                                download=True)
    assert args.N_train + args.N_forge <= len(full_dataset)

    train_dataset = Subset(full_dataset, TRAIN_INDS)
    forge_dataset = Subset(full_dataset, FORGE_INDS)
    ...
    for epoch in tqdm(range(N_EPOCHS)):
        for X, y_true, train_ind in train_loader:
            ...
            if FORGE:
                ...
                forge_loader = DataLoader(dataset=forge_dataset, 
                                        batch_size=args.n, 
                                        shuffle=True,
                                        drop_last=True)
                for X_cand, y_cand, ind_cand in forge_loader:
        ...
        if (epoch+1) % args.ckpt_every == 0:
            # save files 
            torch.save({'epoch': epoch+1, 'train': train_inds, 'forge': forge_inds}, '{}/inds.pkl'.format(tb_folder))
            torch.save({'ori_state_dict': model_ori.state_dict(),
                        'forge_state_dict': model_forge.state_dict()}, 
                    '{}/model-{}epochs.pkl'.format(tb_folder, epoch+1))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ckpt_every", type=int, default=5, help="interval ckpt (epoch)")
    ...
    parser.add_argument("--N_train", type=int, default=30000, help="first N training samples for model_ori")
    parser.add_argument("--N_forge", type=int, default=30000, help="next N samples for model_forge")

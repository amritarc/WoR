...
def sweep(score, x):
    ...
    acc, best_thr = np.max(1-(fpr+(1-tpr))/2), np.argmax(1-(fpr+(1-tpr))/2)
    return fpr, tpr, auc(fpr, tpr), acc, thresholds[best_thr]
...
def generate_ours(keep, scores, check_keep, check_scores, in_size=100000, out_size=100000,
    ...
    print(dat_in.shape, dat_out.shape)
...
def do_plot(fn, keep, scores, ntest, legend='', metric='auc', sweep_fn=sweep, **plot_kwargs):
    ...
    fpr, tpr, auc, acc, best_thr = sweep_fn(np.array(prediction), np.array(answers, dtype=bool))
...
def fig_fpr_tpr(p):
    ...
    plt.savefig(os.path.join(p, "fprtpr.png"))
    plt.show()


if __name__ == '__main__':
    logdir = "exp/mnist-10k/"
    load_data(logdir)
    fig_fpr_tpr(logdir)

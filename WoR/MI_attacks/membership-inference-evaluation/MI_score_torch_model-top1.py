...
def compute_MI_score(
    ...
    top1_dic = {'original': [], 'forged': [], 'labels': []}  # i-th element in each value is i-th top1; total len = num_del
    ...
    for sc_name, model in zip(['original', 'forged'], [model_ori, model_forge]):
        ...
        top1 = _top1(_outputs)
        top1_dic[sc_name] = list(top1)
    
    return top1_dic, [checkpoint_forge['model_dist']] * num_del
...
def main(dataset, model, shadow_model, K_RANGE, Ntrain, M, num_del, n_epochs, eval_split, recompute, 
    ...
    top1diff_name = r"$\Lambda_i(\theta_*)-\Lambda_i(\theta_{-i})$"
    top1div_name = r"$\frac{\Lambda_i(\theta_{-i})}{\Lambda_i(\theta_*)}-1$"
    df_dic = {
        "K": [],
        "top1_labels": [],
        "top1_original": [],
    ...
    for splitK in K_RANGE:
        ...
        output_file = './exp-top1/{}/del-{}-epoch-{}/{}-split-{}.pkl'.format(dataset, num_del, n_epochs, forging_path, eval_split)
        os.makedirs('./exp-top1/{}/del-{}-epoch-{}'.format(dataset, num_del, n_epochs), exist_ok=True)
        ...
        if recompute:
            ...
            thr = compute_or_load_threshold(
                ...
                outfile='./exp-top1/{}/del-{}-epoch-{}/{}-thr.pkl'.format(dataset, num_del, n_epochs, forging_path)
            )
            ...
            for delete_ind in tqdm(range(0, int(Ntrain // num_del))):  # int(Ntrain // num_del)
                ...
                top1_dic, model_dist = compute_MI_score(
                    dataset, dataloader, Ntrain, delete_ind, num_del, eval_split,
                    model, model_ori, model_forge_path, forging_path, device
                )
                top1_labels += top1_dic['labels']
                top1_original += top1_dic['original']
                top1_forged += top1_dic['forged']
                model_dists += model_dist
                exp_id += [delete_ind] * num_del  # [delete_ind] * num_del
            
            # top1_original = np.array(top1_original)
            # top1_forged = np.array(top1_forged)
            ...
            torch.save({
                ...
                'top1_original': top1_original,
                'top1_forged': top1_forged,
            ...
            top1_labels = dic['top1_labels']
            top1_original = dic['top1_original']
            top1_forged = dic['top1_forged']
            model_dists = dic['model_dists']
            num_del = dic['num_del']
            exp_id = dic['exp_id']
        
        df_dic["K"] += [splitK] * len(top1_original)
        df_dic["top1_labels"] += list(top1_labels)
        df_dic["top1_original"] += list(top1_original)
        df_dic["top1_forged"] += list(top1_forged)

        if eval_split == 'diff':
            df_dic["exp_id"] += exp_id
            df_dic[dist_name] += model_dists
            df_dic[top1diff_name] += list(np.array(top1_original) - np.array(top1_forged))
            df_dic[top1div_name] += list(np.array(top1_forged) / np.array(top1_original) - 1.0)
        ...
        #     'TT': sum((top1_original >= thr) * (top1_forged >= thr)),
        #     'TF': sum((top1_original >= thr) * (top1_forged <  thr)),
        #     'FT': sum((top1_original <  thr) * (top1_forged >= thr)),
        #     'FF': sum((top1_original <  thr) * (top1_forged <  thr)),
        # }
    ...
    for splitK in K_RANGE:
        ...
        #     if len(df_exp_id) == 0:
        #         continue
        #     if sum((df_exp_id["top1_original"] >= thr) ^ (df_exp_id["top1_forged"] >= thr)) > 0:
        #         MI_pred_on_DIFF['exist_wrong'] += 1
        #     MI_pred_on_DIFF['total'] += 1

        #     MI_pred_on_DIFF['exist_wrong'] += sum((df_exp_id["top1_original"] >= thr) ^ (df_exp_id["top1_forged"] >= thr))
        #     MI_pred_on_DIFF['total'] += len(df_exp_id["top1_original"])
        ...
        for label in range(N_CLASSES):
            df_label = df_K[df_K['top1_labels'] == label]
            wrong += sum((df_label["top1_original"] >= thr[label]) ^ (df_label["top1_forged"] >= thr[label]))
    ...
    # plt.figure(figsize=(8,5))
    # q1, q3 = np.percentile(df[top1diff_name], [25, 75])
    # low, high = q1 - (q3 - q1) * outlier_scale, q3 + (q3 - q1) * outlier_scale
    # df_tmp = df[(df[top1diff_name] < high) & (df[top1diff_name] > low)]
    # sns.violinplot(y=df_tmp["K"], x=df_tmp[top1diff_name], bw=0.1)
    # plt.savefig('./exp/{}/del-{}-epoch-{}/top1_diff-{}.png'.format(dataset, num_del, n_epochs, forging_path), dpi=300)

    # plt.figure(figsize=(8,5))
    # q1, q3 = np.percentile(df[top1div_name], [25, 75])
    # low, high = q1 - (q3 - q1) * outlier_scale, q3 + (q3 - q1) * outlier_scale
    # df_tmp = df[(df[top1div_name] < high) & (df[top1div_name] > low)]
    # sns.violinplot(y=df_tmp["K"], x=df_tmp[top1div_name], bw=0.1)
    # plt.savefig('./exp/{}/del-{}-epoch-{}/top1_div-{}.png'.format(dataset, num_del, n_epochs, forging_path), dpi=300)

    # quantiles = [0, 1, 2, 5, 10, 25, 50, 75, 90, 95, 98, 99, 100]
    # for col in [dist_name, top1diff_name, top1div_name]:
    #     print('\ncol={}'.format(col))
    #     res = {}
    #     for splitK in K_RANGE:

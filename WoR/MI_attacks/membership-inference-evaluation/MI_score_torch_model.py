...
def compute_or_load_threshold(model_shadow, trainloader, testloader, device, outfile):
    if os.path.exists(outfile):
        ...
        tr_values = _m_entr_comp(_outputs, tr_labels)

        _outputs, te_labels = _model_predictions(model_shadow, testloader, device)
        te_values = _m_entr_comp(_outputs, te_labels)
...
def compute_MI_score(
    ...
    mentr_dic = {'original': [], 'forged': [], 'labels': []}  # i-th element in each value is i-th mentr; total len = num_del
...
def main(dataset, model, shadow_model, K_RANGE, Ntrain, M, num_del, n_epochs, eval_split, recompute, 
    ...
    for splitK in K_RANGE:
        ...
        df_dic["mentr_labels"] += list(mentr_labels)
    ...
    for splitK in K_RANGE:
        ...
        for label in range(N_CLASSES):
            df_label = df_K[df_K['mentr_labels'] == label]
            wrong += sum((df_label["mentr_original"] >= thr[label]) ^ (df_label["mentr_forged"] >= thr[label]))
